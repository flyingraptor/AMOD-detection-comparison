...
angles = [0]
label_version = 'v1.0'              # 'v1.0' or 'v1.5' (We recommend using v1.5)
data_root = '/media/kimsooyeon/D/AMOD2VISIR_UNIT/'         # Important: should be ended with '/'
...
train_pipeline = [
    ...
    dict(type='LoadAnnotations', with_bbox=True, with_mask=False),   # 기본 mask 로딩 off
    dict(type='LoadAMODMasks', mask_prefix='/media/kimsooyeon/D/AMOD2VISIR_UNIT/train_mask/'),   # npy mask 로더
    dict(type='MaskBrightness',                                      # 마스크 기반 밝기 증강
        mode='bilateral',        # 'gaussian' | 'average' | 'median' | 'bilateral' | 'fft' | 'perlin'
        strength=0.75,          # 밝기 변조 강도 (contrast scale)
        blur_sigma=40,          # 저주파 스무딩 크기(px)
        feather_px=5,           # 경계 부드럽게
        per_instance=True,     
        preserve_mean=True,     
        prob=0.7,              
        seed=None),             
